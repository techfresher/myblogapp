...
def index(request):
    ...
    context = {"posts":posts, "top_posts":top_posts, "recent_posts":recent_posts, "subscribe_form":subscribe_form, 
               "subscribe_successful":subscribe_successful, 'featured_post':featured_post, 'website_info':website_info}
...
def post_page(request, slug):
    ...
    context = {'post':post, 'form':form, 'comments':comments, 'is_bookmarked':is_bookmarked, 
               ...
               'recent_post':recent_post, 'tags':tags, 'top_authors':top_authors,
...
#     print("PRINT", request.POST.get("post_id"))
#     post = get_object_or_404(Post, id=request.POST.get('post_id'))
#     if post.comment_count.filter(id=request.user.id).exists():
#         post.comment_count.remove(request.user)
#     else:   
#         post.comment_count.add(request.user)
